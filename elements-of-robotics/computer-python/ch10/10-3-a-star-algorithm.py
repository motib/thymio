"""
  A* shortest path algorithm on a grid map with variable costs
  ...
  A cell contains the distance (initially 0's, -1's for obstacles),
    the cost (usually 1) and the heuristic function
...
def print_grid():
    ...
    for i in range(len(grid)):
        for j in range(len(grid[i])):
            if source == [i,j]:
                print ('[S, '+str(grid[i][j][1])+', '+str(grid[i][j][2])+']  ', end='')
            elif goal == [i,j]:
                print ('[G, '+str(grid[i][j][1])+', '+str(grid[i][j][2])+']   ', end='')
            elif grid[i][j][0] == -1:
                print ('[X      ]', ' ', end='')
            else:
                print (grid[i][j], ' ', end='')
...
# Initialize heuristic function to be distance without obstacles
def init_heuristic():
    global grid, goal
    for i in range(len(grid)):
        for j in range(len(grid[i])):
            grid[i][j][2] = abs(i-goal[0])+abs(j-goal[1])

# Initialize the grid to 0's, -1's for obstacles
def init_grid():
    global grid, source, goal, marked, marks
    # Build grid of zeros and cost 1
    grid = [[[0,1,0] for i in range(6)] for j in range(5)]
    # Set obstacles
    grid[4][2] = [-1,0,0]; grid[3][2] = [-1,0,0]; grid[2][4] = [-1,0,0]
    grid[2][5] = [-1,0,0]; grid[0][2] = [-1,0,0]; grid[1][2] = [-1,0,0]
    # Don't use variable costs for this example
...
def mark_cell(row,col,d):
    ...
    if grid[row][col][0] == 0:
        ...
        # Sort by cost: distance from source + heuristic function
        marked.sort(key=lambda cost: \
          grid[cost[0]][cost[1]][0] + grid[cost[0]][cost[1]][2])
        marks += 1
    else:
        return
