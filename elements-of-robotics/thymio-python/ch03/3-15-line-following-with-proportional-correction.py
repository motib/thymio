...
# Proportional line following with one sensor
...
SCALE = 4             # scaling the motor change
...
def prox():
  ...
  if  state != 0:
      ...
      scaled = motor_change + abs(prox_ground_delta[1])//SCALE
      if  prox_ground_delta[1] > THRESHOLD_HIGH:
         motor_left_target  = motor + scaled
         motor_right_target = motor - scaled
         nf_leds_top(0, 32, 0)
